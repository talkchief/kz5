...
| E cutover rehearsal | **Not run.** Owner authorized a read-only replication; the assistant's session may not contact production, so the owner starts it: `doc/cutover_rehearsal_runbook.md`, step 1 (`scripts/cutover-rehearsal-plan.py`, GET-only sizing). The upgrade-install matrix on production-shaped data is part of it. |
