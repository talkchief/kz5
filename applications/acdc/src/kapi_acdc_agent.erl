...
-define(OPTIONAL_SYNC_RESP_HEADERS, [<<"Call-ID">>, <<"Agent-Call-ID">>
                                    ,<<"Time-Left">>, <<"Process-ID">>, <<"Queues">>]).
...
login_queue(Props) when is_list(Props) ->
    case login_queue_v(Props) of
        'true' -> kz_api:build_message(Props, ?AGENT_HEADERS, [<<"Runtime-Only">> | ?OPTIONAL_AGENT_HEADERS]);
...
%% Member Connect Win
%%------------------------------------------------------------------------------
-define(MEMBER_CONNECT_WIN_HEADERS, [<<"Queue-ID">>, <<"Agent-ID">>, <<"Call">>, <<"Agent-Process-IDs">>]).
