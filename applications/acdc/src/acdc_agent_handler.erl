...
maybe_start_agent(AccountId, AgentId, JObj) ->
    Start = fun() ->
                    case kz_json:is_true(<<"Runtime-Only">>, JObj) of
                        'true' -> maybe_start_selected_queue_agent(AccountId, AgentId, kz_json:get_value(<<"Queue-ID">>, JObj));
                        'false' -> maybe_start_agent(AccountId, AgentId)
                    end
            end,
    try Start() of
        {'ok', Sup} ->
        ...
        {'exists', Sup} ->
            ...
            Sup;
        {'error', _E} ->
            maybe_record_login_failure(AccountId, AgentId, JObj),
            'fail'
    catch
        _E:_R ->
            maybe_record_login_failure(AccountId, AgentId, JObj),
...
-spec maybe_record_login_failure(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) -> 'ok'.
maybe_record_login_failure(AccountId, AgentId, JObj) ->
    case kz_json:is_true(<<"Runtime-Only">>, JObj) of
        'true' -> 'ok'; % Rejected queue enrollment must not log out a running agent.
        'false' -> acdc_agent_stats:agent_logged_out(AccountId, AgentId)
    end.

%% Runtime-only queue login must not enroll an agent or start every saved queue.
%% Re-read enrollment when consuming the command, and preserve an existing
%% agent process (including all its other runtime queues and its current state).
-spec maybe_start_selected_queue_agent(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
          {'ok', pid()} | {'exists', pid()} | {'error', any()}.
maybe_start_selected_queue_agent(AccountId, AgentId, QueueId) ->
    Db = kzs_util:format_account_db(AccountId),
    case {kz_datamgr:open_doc(Db, AgentId), kz_datamgr:open_doc(Db, QueueId)} of
        {{'ok', Agent}, {'ok', Queue}} ->
            Queues = kz_json:get_value(<<"queues">>, Agent, []),
            case is_binary(QueueId) andalso kz_doc:type(Agent) =:= <<"user">>
                andalso kz_doc:account_id(Agent) =:= AccountId andalso kzd_users:enabled(Agent)
                andalso not kz_doc:is_soft_deleted(Agent) andalso is_list(Queues)
                andalso lists:member(QueueId, Queues) andalso kz_doc:type(Queue) =:= <<"queue">>
                andalso kz_doc:account_id(Queue) =:= AccountId andalso not kz_doc:is_soft_deleted(Queue)
            of
                'false' -> {'error', 'not_enrolled'};
                'true' ->
                    case acdc_agents_sup:find_agent_supervisor(AccountId, AgentId) of
                        'undefined' ->
                            case acdc_agents_sup:new(kz_json:set_value(<<"queues">>, [QueueId], Agent)) of
                                {'error', {'already_started', Running}} when is_pid(Running) -> {'exists', Running};
                                Started -> Started
                            end;
                        Sup when is_pid(Sup) -> {'exists', Sup}
                    end
            end;
...
handle_call_event(Category, <<"CHANNEL_DESTROY">> = Name, FSM, JObj, Props) ->
    ...
    acdc_agent_listener:channel_destroyed(Srv, CallId),
...
handle_new_channel_acct(_, AccountId) when not is_binary(AccountId); AccountId =:= <<>> -> 'ok';
handle_new_channel_acct(JObj, AccountId) ->
    CallId = kz_json:get_value(<<"Call-ID">>, JObj),
    handle_new_channel_acct(JObj, AccountId, CallId).

-spec handle_new_channel_acct(kz_json:object(), kz_term:ne_binary(), any()) -> 'ok'.
handle_new_channel_acct(_, _, CallId) when not is_binary(CallId); CallId =:= <<>> -> 'ok';
handle_new_channel_acct(JObj, AccountId, CallId) ->
    MemberCallId = kz_json:get_value([<<"Custom-Channel-Vars">>, <<"Member-Call-ID">>], JObj),
    {Keys, Message} = case kz_call_event:call_direction(JObj) of
        <<"inbound">> -> {[<<"From">>], ?NEW_CHANNEL_FROM(CallId)};
        <<"outbound">> ->
            {[<<"To">>, <<"Request">>], ?NEW_CHANNEL_TO(CallId, MemberCallId)};
        _ -> {[], 'undefined'}
    end,
    lists:foreach(fun(User) -> gproc:send(?NEW_CHANNEL_REG(AccountId, User), Message) end,
                  channel_usernames(Keys, JObj)).
...
handle_destroyed_channel(_, AccountId) when not is_binary(AccountId); AccountId =:= <<>> -> 'ok';
handle_destroyed_channel(JObj, AccountId) ->
    CallId = kz_json:get_value(<<"Call-ID">>, JObj),
    handle_destroyed_channel(JObj, AccountId, CallId).

-spec handle_destroyed_channel(kz_json:object(), kz_term:ne_binary(), any()) -> 'ok'.
handle_destroyed_channel(_, _, CallId) when not is_binary(CallId); CallId =:= <<>> -> 'ok';
handle_destroyed_channel(JObj, AccountId, CallId) ->
    HangupCause = acdc_util:hangup_cause(JObj),
    Keys = case kz_call_event:call_direction(JObj) of
        <<"inbound">> -> [<<"From">>];
        <<"outbound">> -> [<<"From">>, <<"To">>];
        _ -> []
    end,
    lists:foreach(fun(User) ->
                      gproc:send(?DESTROYED_CHANNEL_REG(AccountId, User),
                                 ?DESTROYED_CHANNEL(CallId, HangupCause))
                  end, channel_usernames(Keys, JObj)).

%% SIP addresses are optional in channel events (e.g. a local monitor leg).
%% Only notify users actually identified by the direction-relevant fields;
%% never invent an identity from a caller ID or suppress another valid field.
%% Preserve normalized bare usernames accepted by the previous implementation.
-spec channel_usernames(kz_term:ne_binaries(), kz_json:object()) -> kz_term:ne_binaries().
channel_usernames(Keys, JObj) ->
    lists:usort([User || Key <- Keys,
                        User <- [channel_username(kz_json:get_value(Key, JObj))],
                        User =/= 'undefined']).

-spec channel_username(any()) -> kz_term:api_binary().
channel_username(Value) when is_binary(Value), byte_size(Value) > 0 ->
    case binary:split(Value, <<"@">>, ['global']) of
        [User, Realm] when byte_size(User) > 0, byte_size(Realm) > 0 -> User;
        [User] -> User;
        _ -> 'undefined'
    end;
channel_username(_) -> 'undefined'.
