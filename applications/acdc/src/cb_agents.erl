...
validate_agent_action(Context, AgentId, ?QUEUE_STATUS_PATH_TOKEN, ?HTTP_POST) ->
    case cb_acdc_agent_queue:requested(Context) of
        'true' -> cb_acdc_agent_queue:validate(AgentId, read(AgentId, Context));
        'false' ->
...
validate_agent_action(Context, AgentId, ?QUEUE_STATUS_PATH_TOKEN, ?HTTP_GET) ->
    case cb_acdc_agent_queue:requested(Context) of
        'true' -> cb_acdc_agent_queue:read(AgentId, read(AgentId, Context));
        'false' -> fetch_agent_queues(read(AgentId, Context))
    end;
...
post(Context, AgentId, ?QUEUE_STATUS_PATH_TOKEN) ->
    case cb_acdc_agent_queue:requested(Context) of
        'true' -> cb_acdc_agent_queue:post(AgentId, Context);
        'false' -> post_queue_membership(Context, AgentId)
    end;
post(Context, AgentId, ?RESTART_PATH_TOKEN) ->
    publish_restart(Context, AgentId),
    crossbar_util:response(kz_json:new(), Context).

-spec post_queue_membership(cb_context:context(), path_token()) -> cb_context:context().
post_queue_membership(Context, AgentId) ->
