...
| P0-19 | ACTIVE — source/HTTP/primary-CAS PASS; not deployed | Installer-owned candidate preserves the validated revision and rejects invalid revisions before writes. Nine controlled groups and ten real private-Cowboy HTTP cases pass; both HTTP races reproduce204 instead of409 on baseline. Native primary CouchDB save path preserves concurrent edits in b3c7e1/2a0330, with controlled routing/cache/publication. Full deployed Crossbar authorization/route acceptance, cluster behavior and token lifecycle remain open; restricted-dashboard fixture admission stays closed. No cascade/queue-activation transaction claim. See `doc/crossbar_soft_delete_revision.md`. |
| P0-20 | ACTIVE — source + real-primary regression PASS; not deployed | Native single Couch deletion passed through HTTP201 bulk rows containing conflicts as success. Baseline real-Couch probe687399/f72f2d reproduces false success. Required installer patch normalizes only single-delete response rows, preserving valid/legacy shape and transport errors; batch and non-Couch paths unchanged. Nine focused groups pass; baseline fails six. Candidate15-module build and actual primary soft/hard conflict/body/no-success-hook checks pass b3c7e1/2a0330; two owned test docs removed with exact revisions/absence checks. Three tiny synthetic databases retained, two with baseline fixture documents; no customer data or services changed. Deployment/full HTTP/cluster acceptance open. See `doc/couch_single_delete_result.md`. |
...
| DASH-05 | ACTIVE — current specs published | Versioned summary/detail DTOs and native Blackhole message/lifecycle/account/queue contracts refreshed at `/apis` (52790), with11 exact HTTP asset hashes, no-store, redirect308 and missing404 verified. Catalog50258 includes358 paths/653 operations; complete offline/deterministic/tamper suite32709 passed. Publication rollback remains recoverable at `/usr/local/src/kazoo5-installer/api-docs-rollback.UW8oYV/previous`. Actual detail/summary browser call transitions passed a0d26078/f965e9fb; restricted-token isolation remains open. Planned history is not callable. |
