...
| P0-19 | ACTIVE — source regression PASS; not deployed | Native Crossbar refreshed the revision before soft-saving an older validated body, allowing concurrent edits to be overwritten. Installer-owned candidate preserves the validated revision and rejects invalid revisions before writes. Nine controlled production-module groups pass; pinned baseline fails seven (including actual stale-body overwrite), with two hard-delete groups unchanged. Ten modules compile without TEST/export_all. Real HTTP412 versus datastore409, CouchDB concurrency, deployment and token invalidation remain open; restricted-dashboard fixture admission stays closed. No cascade/queue-activation transaction claim. See `doc/crossbar_soft_delete_revision.md`. |
