...
| INST-04 | ACTIVE — deployment acceptance | Monster production `preloadApps`/`preloadedApps` mismatch corrected. Guarded build/readback passed: 1,931 files, 465 templates, 16 canonical preloads. Actual unauthenticated artifact boot and mocked queue-login flow passed, not live authenticated API proof. The five-phase 24-suite checkpoint omitted `test-install-kazoo5.sh`; its stale ALL assertion needs separate acceptance. Queue recovery fix `8c11030` passed 15 source-browser cases but needs a new matched artifact. See `doc/installer_regression_acceptance_20260906.md`; adoption, matched backend and clean-server dependencies remain unverified. |
