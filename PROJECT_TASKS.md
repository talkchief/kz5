...
| P0-12 | ACTIVE — callback media deadline | Review found synchronous auxiliary metadata lookup can exceed the remaining menu/21-second feedback deadline and delay hangup/usurp processing. Built-in preflight already resolves all42 assets: retain its three auxiliary paths; resolve legacy auxiliary defaults before queue entry; timed unavailable/retry/alternate branches must use only cached exact paths and fail quietly when absent. Add poisoned/slow-resolver and short-budget tests while preserving ownership and completion cases. Existing81 passing callback tests do not prove this missing datastore-bound case. |
