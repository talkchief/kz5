...
| P0-03 | ACTIVE — EN registration/retry diagnostic verified; single-key UX open | Actual installed Gemini6+1 registration passes f99df0/90042a: full success phrase before BYE, first return attempt deliberately unanswered, durable retry_wait and second reciprocal native bridge, agent ready afterward. Fixture retained, not full cleanup/production acceptance. The requested single-key6 behavior remains distinct: current registration requires1 even with alternatives disabled; focused source correction is in progress. Invalid user return-number configuration, real MOH and all-language acceptance remain open. Earlier private immediate-audio candidate is unaccepted and must not be deployed based on this normal-path diagnostic. See doc/acdc_gemini_callback_retry_acceptance_20260907.md. |
