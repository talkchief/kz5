...
| INST-12 | OPEN — build identity | Canonical tracked ACDC is included in both installer build paths, but Make uses timestamps and production BEAM validation checks TEST defines, not source identity. Add regression coverage and content-aware invalidation for changed/restored sources with older mtimes, plus validation before same-invocation build reuse. Do not claim source-to-deployed-BEAM identity from active services or timestamps alone. |
