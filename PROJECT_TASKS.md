...
| P0-25 | ACTIVE — indefinite ACDC loading | September 7 operator reports opening ACDC leaves the loading animation indefinitely. Source review found unbounded GET settlement; add bounded read/error/retry behavior without replaying writes or leaking stale account results. Root browser diagnosis also timed out at native shell readiness, so do not claim the source timeout explains every case; inspect real browser startup/controller state and errors. Test first open, reopen, tab/account switch, unavailable API and late replies against served build. |
...
| DASH-10 | OPEN — caller display/API | September 7 user requests caller Number/Name instead of UUID in selected-queue live call rows. Add bounded nullable caller-name/number fields from authorized selected-queue observations through collector, native reply codec, public DTO, UI and OpenAPI; preserve internal call_id for identity/actions. Source audit found existing call_stat caller fields lack privacy provenance: manager caller_id lookup does not explicitly apply privacy flags. Establish tested upstream privacy-safe fields/marker first; old unmarked calls yield unavailable, not an invented anonymous label. Proposed limits256/64 UTF-8 bytes, selected queue only, control/malformed/oversized values null without dropping occupancy. Include identity in replica agreement; retain internal ID for actions and escape visible text. Keep PII out of overview scans and WebSocket hints. Test privacy, tenant/queue isolation, mixed versions, replica disagreement and actual browser display. |
