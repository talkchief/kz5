...
| INST-06 | OPEN — deployment | Publish reviewed matching source/backend/UI with exact backups; preserve unselected apps/config/customer data and runtime queue memberships/pauses. Read-only probe `29950` confirms old FSM and missing queue-login/recovery interfaces: the new bundle requires a coherent ten-module ACDC release, drained work/admission control and tested process-state migration or restoration, not a six-module hotload. Compiled UI publication remains held. See `doc/acdc_coherent_upgrade_readiness.md`. |
