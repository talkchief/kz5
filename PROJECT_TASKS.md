...
| VOICE-05 | OPEN — source/runtime parity | Fresh live probe on September 6 confirms loaded acdc_announcements imports Gemini resolver functions and its loaded MD5 matches its on-disk BEAM; account 302ae5a70c403124f764cbc54229cfcd resolves the absent/default English offer-6 to a Gemini/Sulafat ID. Canonical tracked acdc_announcements.erl currently has no Gemini resolver calls. Reconcile that runtime/source discrepancy before any rebuilt ACDC deployment, preserving new scheduler fixes and explicit customer overrides. Do not mistake an imported asset, loaded helper, or old deployment note for effective queue selection. English capability still reports full_language_ready=false and callback_runtime_ready=false. |
...
| VOICE-06 | ACTIVE — required UI contract | Exactly one Queue language dropdown: EN, HE, FR, ES, AR. Exactly one built-in female Gemini voice per language, no voice picker, per-prompt selector or custom-recording editor. Selection applies consistently to position/wait announcements, callback offers, menus, number readback, confirmation/success, returned-call confirmation, invalid input and unavailable-action responses. Existing media documents must not be deleted. Clear obsolete queue prompt references when adopting built-in language defaults; do not silently keep an override that defeats the selection. |
| VOICE-07 | ACTIVE — immutable WAV generation | Generate missing fixed, numeric and auxiliary phrases once using Gemini, reusing valid existing Sulafat recordings. Produce PCM16 mono masters and telephony-rate WAVs; retain transcripts, language/voice/model provenance, hashes and duration/clipping checks. Cover agent-invalid_choice, menu-invalid_entry and cf-enter_number in all five languages, plus complete position/telephone numeric composition. No native robotic SAY or mixed-voice fallback in the finished standard pack. Never regenerate during install, account/sub-account creation, queue editing or calls. |
...
| VOICE-09 | OPEN — deployment acceptance | Prove the selected queue uses the expected Gemini assets for position, independent callback interval, key-6 menu/success, invalid/alternate-number responses and returned-call confirmation in each language. Listen for natural female speech, correct words/numbers/language, no clipping or silence; preserve queued calls and agent state. Deploy matching source/backend/UI/assets through the installer after rollback/admission checks. Imported files or successful TTS generation alone do not close this task. |
...
| API-03 | ACTIVE — documentation | Repository catalog validates 356 paths / 651 operations; deterministic regeneration/tamper, queue-login, members/device and Blackhole contract checks pass. Assets ab9d78a published at /apis (51631): all 12 files match HTTP readback, redirect/404/no-store verified, rollback recorded in doc/api_developer_portal.md. Real isolated Chromium rendering passes (95890); HTTPS and live endpoint acceptance remain separate, with proposals/gaps explicit. |
