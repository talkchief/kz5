...
- **Active supervision controller partition — run3 PASS after readiness fix:**
  ...
  Listen audio/privacy survived the real interruption, and same-VM broker
  registration recovered; the first supervisor stop then returned503. Exact
...
| VOICE-06 | DEPLOYED — five-choice selection AND SAVE/RELOAD PASS; wider call acceptance open | One Queue language dropdown offers exactly EN/HE/AR/FR/ES with separate callback/position interval controls. Earlier browser0b7f2c proves selection/cancel. Later isolated-fixture browser97969/d810a2 proves actual create/save for all five languages and fresh readback, then callback disable and reopening ES with generic17/callback30 intervals. No copied-company writes, browser errors or stuck indicator. Inherited/unsupported settings now survive unrelated edits (070f42). Native new-child reseller resolution passes3d60a1; inherited live-call audio, every prompt branch and native pronunciation remain separate. See doc/queue_browser_save_acceptance.md and doc/media_reseller_language.md. |
...
| VOICE-08 | DEPLOYED CANONICAL SOURCE — distributed/call-context acceptance open | ACDC, prerecorded maps, shared WAVs, import/readback logic and installer wiring are tracked in kz5; .44 has no nested ACDC Git metadata. Normal apps/eCallMgr deployment and actual runtime media/map proof passed; all-five callback success tests retain the selected language. Native fresh-child inherited language/media resolution passes3d60a1 and core installer patch replay is verified. Inherited live calls, every response path, cross-node propagation and separated apps-node installation remain required. A single-host result does not close distributed deployment or all language-context paths. |
