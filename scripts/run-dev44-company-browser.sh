...
set -Eeuo pipefail
[[ $EUID == 0 && ( $# == 0 || ( $# == 1 && ( $1 == --callflows-users || $1 == --queue-create-form ) ) || ( $# == 2 && ( $1 == --queue-create-save || $1 == --queue-login-check ) && $2 == --allow-fixture-writes ) ) ]] || { echo 'Usage: sudo bash scripts/run-dev44-company-browser.sh [--callflows-users|--queue-create-form|--queue-create-save --allow-fixture-writes|--queue-login-check --allow-fixture-writes]' >&2; exit 64; }
