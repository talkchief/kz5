...
function applyOverlays(spec, root) {
    ...
    // Queue validation schema comes from a patched upstream checkout. Keep
    // current ACDC interaction documentation in this tracked kz5 overlay so
    // rebuilding /apis does not depend on an untracked local schema edit.
    const callbackProperties = schemas.queues.properties.callback.properties;
    schemas.queues.properties.announcements.properties.language.description = 'Optional lowercase BCP 47 prompt language override. When omitted, the caller/account prompt language is used. Newly admitted calls snapshot their effective language for position announcements, callback offers and responses, including resumed announcement workers and callback retries. Later queue edits affect new admissions; entering another queue takes a new snapshot. Legacy calls without a snapshot retain the prior worker fallback behavior. Built-in audio uses installed recordings, never runtime voice generation.';
    schemas.queues.properties.announcements.properties.language.description += ' Native account fallback precedence: account media.default_language, account language, direct reseller media.default_language, direct reseller language, then the caller/system default. Explicit call/endpoint language and the queue override remain authoritative. With media.support_account_overrides=false, the native account resolver uses the system default. Reseller lookup failure retains the caller/system fallback; there is no recursive reseller-chain lookup or account-document migration.';
    callbackProperties.allow_alternate_number.description = 'Default false: the queue entry_key requests registration of the valid current caller-ID destination without an extra confirmation digit. When true, entry_key opens a choice menu: 1 confirms the current number, or 2 collects an alternate number, followed by #, readback and 1 confirmation. An invalid current number goes directly to alternate collection only when true. All destinations remain subject to authorization and durable registration before success audio or hangup; returned-call confirmation 1 is separate.';
    callbackProperties.entry_key.description = 'Single DTMF digit selecting callback while waiting (default 6). After a correlated queue pause, valid current caller ID with allow_alternate_number=false requests durable registration immediately, without another 1. With alternatives enabled it opens the destination-choice menu. This does not bypass destination authorization or returned-call confirmation 1.';
    callbackProperties.confirmation_timeout.description = 'Seconds allowed for the returned caller to press DTMF 1 after the confirmation recording finishes (3–30, default 10). Digit 1 may also interrupt playback. The response window begins once on the exact call-bound playback completion; duplicate or unrelated events cannot extend it. Playback has a separate 30-second watchdog: missing completion or playback failure ends this attempt through the normal cleanup/retry policy, without connecting an agent. Installation and calls use prerecorded media, never runtime voice generation.';
    ...
    const inputs = [queueSource, monitorSource, agentSource, 'applications/acdc/src/acdc_callback_store.erl',
        'applications/acdc/src/acdc_callback_internal.erl', 'applications/acdc/src/acdc_callback_policy.erl',
        'applications/acdc/src/acdc_callback_caller.erl', 'applications/crossbar/src/modules/cb_channels.erl']
    ...
    const internalCallback = 'Account-local extension callbacks: numeric destinations of 1–6 digits are checked against an exact callflow in the same account. A terminal user/device callflow may return to enabled account-owned SIP devices; ambiguous, disabled/DND, forwarded, cross-account or changed targets fail closed. Patterns, catch-all and multi-action callflows are not supported by this path. The target identity is pinned privately during registration and resolved again on every attempt; no new public routing fields are required or accepted. Internal callbacks use native resource origination over AMQP, not carrier hunting. The configured outbound authority and internal call restrictions still apply. An absent exact internal route retains the existing external-number policy; a failed or unsupported matched route never falls back to a carrier. Source and runtime request-construction checks do not prove mobile/phone ringing, confirmation or retry acceptance.';
    const callbackSelection = 'Current source contract: with callback.allow_alternate_number=false (default), entry_key (default 6) selects the valid current caller-ID destination and requests registration after a correlated queue pause; no additional registration digit 1 is required. With alternatives enabled, entry opens a choice menu: 1 selects the valid current number, or 2 collects an alternate followed by #, readback and 1 confirmation. Invalid current caller ID goes directly to alternate collection only when alternatives are enabled; otherwise no callback is registered and the caller returns to the live queue. The queue authorizes and durably persists registration before success audio and original-leg teardown. The returned call separately requires 1 before agent connection. Built-in EN/HE/FR/ES/AR WAVs are authored once with Gemini and packaged in the release; installation, startup, account/queue creation and live calls do not invoke Gemini. Source documentation is not deployment or single-key live acceptance evidence.';
    ...
    schemas.CallbackPublic = {...object({id: {type: 'string', pattern: '^acdc-callback-[a-f0-9]{64}$'}, queue_id: hex,
        ...
        attempts: {type: 'integer', minimum: 0}, enqueued_at: {type: 'integer', description: 'Kazoo Gregorian timestamp in seconds, not Unix epoch.'}, enqueue_sequence: {type: 'integer'}, priority: {type: 'integer'}, language: {...text, description: 'Immutable callback language captured from the admitted call after its queue language override. Later queue edits do not change this reservation language or its returned-call confirmation. Retries restore this saved language; legacy reservations without it retain the original call language. Built-in audio uses installed prerecorded assets, never runtime synthesis.'},
    ...
    const list = operation(qi + '/callbacks', 'get', 'List durable callback reservations', queueSource, {description: 'Bounded, queue-scoped list. No public callback-create endpoint exists: only the trusted live queued caller selection and authorized durable-registration workflow may reserve a position. Follow next_cursor from the envelope; do not synthesize cursors. ' + callbackSelection, responses: {200: response('Bounded page', object({status: text, data: {type: 'array', items: ref('CallbackPublic')}, page_size: {type: 'integer', maximum: 100}, next_cursor: {type: 'string', maxLength: 2048}, request_id: text}, ['data'])), ...defaultErrors}});
