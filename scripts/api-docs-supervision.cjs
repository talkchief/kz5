...
const proof = 'Actual distributed SIP/RTP acceptance passed on 2026-09-09, with a new actual-call retest passing on 2026-09-10: Listen/eavesdrop, Whisper, Barge and Join, using separate applications, controllers, Kamailio and FreeSWITCH roles in an isolated lab. Synthetic tones verified both permitted audio and forbidden leakage before/after keypad 3, authorization denials, and supervisor-only stop with the original bridge surviving. The September 10 run also verified these audio properties while new media admission was fenced; existing conversations survived and admission reopened successfully afterward. A separate September 9 controller-broker partition campaign also passed. This is real call/audio evidence, not only HTTP 202. It does not certify media-node failover, full coordinated upgrades or indefinite production load. Repository evidence: doc/channel_monitor_acceptance.md; latest private native run /var/log/kazoo-monitor-acceptance-xD4WRr on dev44.';
const mainProof = 'Main development deployment also passed actual calls on 2026-09-10: the normal kazoo-apps/ecallmgr installer activated source 4cd9953 on dev44, then Listen/eavesdrop, Whisper, Barge and Join passed independent RTP privacy analysis, five authorization negatives per mode and supervisor stop202 with the original bridge surviving. Final cleanup confirmed zero media sessions and no retained fixture. Main evidence: /var/log/kazoo-monitor-acceptance-WTrjrC; deployment and retained failed attempts: doc/main_dev_runtime_promotion.md. This closes the main applications/controller promotion and healthy supervision gate, not full coordinated cluster upgrade/rollback.';
...
<h2>Errors and safety</h2><p>${escape(errors)}</p><section id="testing"><h2>Actual-call testing</h2><p>${escape(mainProof)}</p><p>${escape(proof)}</p></section>
</main></body></html>\n`;
}
