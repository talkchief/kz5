...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    const k = {'--node': 'node', '--receipt': 'receipt', '--fixed-pack': 'fixed', '--completion-pack': 'completion', '--tool-dir': 'toolDirectory'}[argv[i]];
...
function render(template, expected, mode, node) {
  ...
  assert(Array.isArray(expected) && expected.length === 165);
...
function validateResult(stdout, mode) {
  ...
  const m = value.match(/^\{ok,\{ok,\{gemini_mapping_verified,(activate|check),165,330,(\d+),no_database_writes\}\}\}$/);
  assert(m && m[1] === mode && Number(m[2]) <= 330 && (mode !== 'check' || Number(m[2]) === 0), 'Unconfirmed mapping result');
  return Number(m[2]);
}
function execute(o, {spawn = cp.spawnSync, temporaryParent = os.tmpdir(), templateDirectory = __dirname} = {}) {
  ...
  const receipt = protectedReceipt(o.receipt), plan = importer.loadPlan(o.fixed, o.completion, LOCALES);
  ...
  try {
    ...
    const missingBefore = validateResult(result.stdout, o.mode);
    return {schema_version: 1, mode: o.mode, node: o.node, documents_verified: 165, mappings_verified: 330,
