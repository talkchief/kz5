...
function validateReceipt(receipt, plan) {
  assert(Array.isArray(plan) && [165, 210].includes(plan.length) && new Set(plan.map(p => p.id)).size === plan.length,
  ...
  assert(receipt.schema_version === 1 && receipt.owner === IMPORTER_OWNER &&
    receipt.verified === count && receipt.queue_configuration_changed === false &&
  ...
  for (const key of ['created', 'preserved']) assert(Number.isSafeInteger(receipt[key]) &&
    receipt[key] >= 0 && receipt[key] <= count, 'Invalid media receipt count');
  assert(receipt.created + receipt.preserved === count, 'Inconsistent media receipt totals');
  assert(Array.isArray(receipt.prompts) && receipt.prompts.length === count, 'Incomplete media receipt');
...
function main(argv) {
  ...
  for (let i = 0; i < argv.length; i += 2) {
    const key = {'--fixed-pack': 'fixed', '--completion-pack': 'completion', '--supplemental-pack': 'supplemental'}[argv[i]];
    assert(key && !options[key] && argv[i + 1] && path.isAbsolute(argv[i + 1]),
      'Explicit absolute asset directories are required');
  ...
  validateReceipt(JSON.parse(bytes.toString('utf8')), importer.loadPlan(options.fixed, options.completion, LOCALES, options.supplemental));
}
