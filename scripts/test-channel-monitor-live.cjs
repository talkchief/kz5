...
let partitionEnabled=false, queuePartitionEnabled=false, queueCallsEnabled=false, queueFaultEnabled=false, mediaFenceEnabled=false, controllerFault=null;
let mainDev=null;
...
async function main(args) {
    ...
    if(args[0]==='--distributed') {
        ...
        if(args[0]==='--queue-fault') {queueFaultEnabled=true;args=args.slice(1);}
        if(args[0]==='--media-fence') {mediaFenceEnabled=true;args=args.slice(1);}
        assert([partitionEnabled,queuePartitionEnabled,queueCallsEnabled,queueFaultEnabled,mediaFenceEnabled].filter(Boolean).length<=1,'Select one call profile');
    ...
    try {
        ...
        try {
            ...
            if(queuePartitionEnabled||queueCallsEnabled||queueFaultEnabled)await queueContext().run({partition:queuePartitionEnabled,serviceFaultMode:queueFaultEnabled});
        ...
        log((queuePartitionEnabled?'Queued applications-partition recovery passed.':queueFaultEnabled?'Queued service-fault recovery passed.':queueCallsEnabled?'Two queued calls passed.':'All four modes passed.')+' Private synthetic evidence: '+runDir);
