...
erlc -DTEST -Werror +warn_missing_spec \
    ...
    "$project_root/applications/acdc/src/acdc_gemini_prompts.erl" \
...
# The prompt fixture used to be found in the application's ebin. Deployed BEAMs
# are now built without TEST code, so this runner compiles its own copy.
erlc -Werror -I "$project_root/applications/acdc/src" -I "$project_root/applications/acdc/include" -pa "$test_dir" -o "$test_dir" \
    "$project_root/scripts/erlang-tests/acdc_gemini_prompts_tests.erl" \
