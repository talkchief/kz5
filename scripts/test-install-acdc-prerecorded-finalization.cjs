...
function fixture() {
    const files = new Map(), events = [], state = {configured: 'undefined', counter: 0};
    const put = (f, b) => files.set(f, Buffer.isBuffer(b) ? b : Buffer.from(typeof b === 'string' ? b : JSON.stringify(b)));
    put(target, {schema_version: 1, backend_mode: 'legacy', languages: {'en-us': {native_speaker_review: false}}});
    ...
    const fakeFs = {
        ...
        lstatSync: f => { assert(files.has(f)); return {isFile: () => true, isSymbolicLink: () => false,
    ...
    function execute(mode = '--install') {
        events.length = 0; const messages = [], processStub = {argv: ['node', '-', mode, '/fixture/scripts',
            '/fixture/root', '/fixture/config', 'example.invalid', account, 'a'.repeat(64), 'b'.repeat(64)], exitCode: 0};
        ...
        vm.runInNewContext(code, {Buffer, process: processStub, console: {log: m => messages.push(m), error: m => messages.push(m)},
            require(name) { if (name === 'node:fs') return fakeFs; if (name === 'node:path') return path;
                ...
                if (name.endsWith('/validate-acdc-language-capabilities.cjs')) return {assertLanguageCapabilities: x => x};
