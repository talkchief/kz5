...
async function runtime(action,run){
    assert(['edit','verify','restore'].includes(action)&&process.getuid()===0&&path.isAbsolute(run)
    ...
    let receipt=action==='edit'?null:JSON.parse(privateRead(file));
...
if(require.main===module)runtime(...process.argv.slice(2)).catch(()=>{console.error('Callback language case failed; inspect protected receipt, no blind retry or overwrite.');process.exitCode=1;});
