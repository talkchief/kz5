...
function diagnose(run){
    ...
    const receipt=JSON.parse(read('callback-language-edit.json')),bridge=JSON.parse(read('retry-bridge-evidence.json'));
    assert(receipt.account==='8310dc3170a18de37f205d0da172df65'&&receipt.state==='restored'
        &&receipt.before.queue.announcements.language==='en-us'&&receipt.after.queue.announcements.language==='fr-fr'
    ...
    return {scope:'additive-language-payload-diagnosis',language:'en-us',queue_language_during_retry:'fr-fr',
...
if(require.main===module){try{
    const [run]=process.argv.slice(2);assert(process.argv.length===3);
    const result=diagnose(run);
    fs.writeFileSync(path.join(run,'callback-language-payload-diagnosis.json'),JSON.stringify(result,null,2)+'\n',{flag:'wx',mode:384});
