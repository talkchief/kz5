...
-define(MODS, [kapps_config, ecallmgr_fs_acls, ecallmgr_fs_nodes, freeswitch, kz_network_utils, ecallmgr_util]).

mocks(Node) ->
    %% Address parsing stays real; only name resolution is controlled.
    [ok = erpc:call(Node, meck, new, [M, mock_options(M)]) || M <- ?MODS],
    E = fun(M, F, Fun) -> ok = erpc:call(Node, meck, expect, [M, F, Fun]) end,
...
forwarded() ->
    ...
    %% ERL_LIBS is inherited; only the private build directory must be added.
    ...
    try
        ...
        Keeper = erpc:call(Node, erlang, spawn, [fun() -> true = register(ecallmgr_fs_nodes, self()), receive stop -> ok end end]),
        ok = mocks(node()), ok = mocks(Node),
        try
            Out = capture(fun() ->
                no_return = ecallmgr_maintenance:allow_carrier(<<"bill">>, <<"95.217.1.19/32">>),
            ...
            ?assertEqual(0, writes(node())),
            ?assertEqual(1, erpc:call(Node, meck, num_calls, [kapps_config, set_node, '_'])),
            %% The mocked list is empty, so the forwarded removal has nothing to write.
            ?assertEqual(1, erpc:call(Node, meck, num_calls, [kapps_config, set_default, '_'])),
            ?assertNotEqual(nomatch, binary:match(Out, <<"running remove_acl on ", (atom_to_binary(Node))/binary>>)),
            ?assert(erpc:call(Node, meck, called, [kapps_config, set_node, [<<"ecallmgr">>, <<"acls">>, '_', Node]])),
        ...
        after unmock(node()), unmock(Node), Keeper ! stop end
