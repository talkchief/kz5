...
cleanup() {
    rm -f -- "$editor_output/cb_queues.beam" "$editor_output/cb_acdc_queue_editor.beam" "$editor_output/acdc_queue_editor_tests.beam"
    rmdir -- "$editor_output"
}
trap cleanup EXIT
cd "$editor_root"
...
erlc +debug_info -Werror -o "$editor_output" scripts/erlang-tests/acdc_queue_editor_tests.erl
erl -pa "$editor_output" -noshell -eval 'case eunit:test(acdc_queue_editor_tests, [verbose]) of ok -> halt(0); _ -> halt(1) end.'
