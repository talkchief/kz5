...
cleanup() {
    rm -f -- "$editor_output/cb_queues.beam" "$editor_output/cb_acdc_queue_editor.beam" "$editor_output/acdc_queue_editor_tests.beam" "$editor_output/acdc_editor_manifest_path_tests.beam"
    rmdir -- "$editor_output"
}
trap cleanup EXIT
cd "$editor_root"
...
erlc +warn_export_all +warn_unused_import +warn_unused_vars +warn_missing_spec -Werror \
    ...
    applications/acdc/src/cb_queues.erl applications/acdc/src/cb_acdc_queue_editor.erl
...
erlc +debug_info -Werror -o "$editor_output" scripts/erlang-tests/acdc_queue_editor_tests.erl scripts/erlang-tests/acdc_editor_manifest_path_tests.erl
