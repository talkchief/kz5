...
cleanup() {
    rm -f -- "$test_dir/kapi_call.beam" "$test_dir/acdc_callback_recovery_io.beam" \
        "$test_dir/acdc_callback_recovery_io_tests.beam" "$test_dir/acdc_callback_store.beam"
    rmdir -- "$test_dir"
}
trap cleanup EXIT
cd "$project_root"
...
erlc -DTEST -Werror +warn_missing_spec -I applications/acdc/src -I applications/acdc/include \
    ...
    applications/acdc/src/acdc_callback_recovery_io.erl applications/acdc/src/acdc_callback_store.erl
