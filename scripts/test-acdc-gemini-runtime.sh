#!/usr/bin/env bash
# Replay the installer's pinned ACDC base + default aggregate patch privately.
# Never compile staged applications/acdc/src media/language implementations.
set -Eeuo pipefail
...
[[ -f $patch_file && -f $installer_file && -d $acdc_repository ]] || {
    printf '%s\n' 'The project must contain the installer, default patch, and pinned local ACDC checkout.' >&2; exit 2;
}
# Parse one literal pinned revision; do not source the privileged installer or
# evaluate its environment. Local archive replay requires no GitHub access.
...
git -C "$acdc_repository" cat-file -e "${pinned_ref}^{commit}" || {
    printf '%s\n' 'Pinned ACDC commit is unavailable locally; prepare dependencies before running this offline suite.' >&2; exit 2;
}
...
NODE
git -C "$acdc_repository" archive "$pinned_ref" | tar -xf - -C "$test_dir/source"
git -C "$test_dir/source" apply --check "$test_dir/integration.patch"
git -C "$test_dir/source" apply "$test_dir/integration.patch"
...
printf 'PASS pinned ACDC %s; default patch SHA-256 %s; no staged language source or live writes.\n' "$pinned_ref" "$patch_hash"
