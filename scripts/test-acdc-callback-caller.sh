...
cleanup() {
    rm -f -- "$test_dir/acdc_language.beam" "$test_dir/acdc_callback_caller.beam" "$test_dir/acdc_callback_caller_tests.beam"
    rmdir -- "$test_dir"
}
trap cleanup EXIT
cd "$project_root"
...
erlc -DTEST -Werror +warn_missing_spec -I applications/acdc/src -I applications/acdc/include \
    ...
    applications/acdc/src/acdc_language.erl applications/acdc/src/acdc_callback_caller.erl
